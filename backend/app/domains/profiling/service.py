...
from app.domains.profiling.schemas import DatasetProfile, DatasetSummary, ColumnDetails, NumericStats, CategoricalStats, DateStats, BooleanStats
...
class ProfilingServiceImpl(IProfilingService):
    ...
    def _clean_json(self, obj: Any) -> Any:
        if isinstance(obj, dict):
            ...
            return self._clean_json(obj.model_dump())
        elif pd.isna(obj):
            return None
    ...
    def generate_profile(self, db: Session, session_id: str, progress_callback: Callable[[int], None] = None) -> DatasetProfile:
        ...
        cache_path = self.storage.get_cache_path(session_id, "profile.json")
        ...
        logger.info(f"Generating profile for session {session_id}")
        ...
        try:
            ...
            for idx, col in enumerate(df.columns):
                ...
                if idx % max(1, len(df.columns)//4) == 0:
                    if progress_callback:
                        progress_callback(min(90, 25 + int((idx/len(df.columns))*65)))
    ...
    def get_column_profile(self, db: Session, session_id: str, column_name: str) -> ColumnDetails:
        profile = self.generate_profile(db, session_id)
