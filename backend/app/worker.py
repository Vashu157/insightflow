...
from app.domains.shared.logging import current_job_id, current_session_id
...
class KafkaWorker:
    ...
    async def setup(self):
        ...
        self.consumer = AIOKafkaConsumer(
            ...
            enable_auto_commit=False,  # We will manually commit after successful execution
    ...
    async def run(self):
        ...
        try:
            while not self.shutdown_event.is_set():
                # Use getmany to poll, this respects the shutdown event check loop
                # rather than blocking forever on __anext__
    ...
    async def process_message(self, msg):
        ...
        try:
            ...
            # Mark event as seen immediately within a transaction
            ...
            if result.rowcount == 0:
                logger.info(f"Job {event.job_id} is not QUEUED (could be already RUNNING/COMPLETED). Skipping.")
                await self.consumer.commit()
                return

            # 3. Execution execution
            def update_progress(pct: int):
                # We need a new session for progress updates to not mess up the main one,
                # or just use the same session and commit.
            ...
            # Commit offset at the very end of processing
            await self.consumer.commit()
